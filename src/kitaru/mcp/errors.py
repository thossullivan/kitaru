...
from kitaru.client.exceptions import APIError, parse_mastra_replay_reason
...
def map_exception(error: BaseException) -> MCPToolError:
    ...
    if isinstance(error, APIError):
        ...
        details = None
        if status == 409:
            reason = parse_mastra_replay_reason(error.detail)
            if reason is not None:
                details = {"reason": reason}
        return MCPToolError(code, messages[code], retryable=retryable, details=details)
